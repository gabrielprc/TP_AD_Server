...
public class Viaje extends PersistentObject {
	...
	public Viaje(Vehiculo v, Seguro s, ViajeView vi) {
		...
		fechaLlegada = DateUtils.parseDate(vi.getFechaLlegada());
		fechaSalida = DateUtils.parseDate(vi.getFechaSalida());
	...
	public Document generarXml() {
		...
		try {
			...
			eFechaLlegada.appendChild(doc.createTextNode(DateUtils.invParseDate(fechaLlegada)));
			...
			for (ItemCarga ic : cargas) {
				...
				eFecha.appendChild(doc.createTextNode(DateUtils.invParseDate(ic.getFecha())));
